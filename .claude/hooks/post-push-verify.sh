...
# ─────────────────────────────────────────────────────────────────────────────
# Case 2: PR merge to main (production deployment)
# ─────────────────────────────────────────────────────────────────────────────
elif echo "$COMMAND" | grep -qE 'gh\s+pr\s+merge'; then
  # Check if merging to main/master (production)
  # gh pr merge can target main explicitly or default to it
  if echo "$COMMAND" | grep -qE '(--branch\s+main|--branch\s+master|-B\s+main|-B\s+master)' || \
     ! echo "$COMMAND" | grep -qE '(--branch|-B)'; then
...
ENDJSON
    ...
    # Merging to a non-main branch, no production concern
    echo '{}'
  fi
