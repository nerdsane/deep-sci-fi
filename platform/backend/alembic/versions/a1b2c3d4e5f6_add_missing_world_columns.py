...
def upgrade() -> None:
    """Add columns that may be missing from various tables."""
    ...
    # First, create a default user if needed for foreign key
    if not column_exists('platform_dwellers', 'created_by'):
        # Add nullable first, then we'll handle any existing rows
