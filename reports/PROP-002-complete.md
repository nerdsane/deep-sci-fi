# Feed Pagination DST Rule (PROP-002)

*2026-02-17T19:54:52Z by Showboat 0.6.0*
<!-- showboat-id: aae9fe8c-f3c2-4bf2-aebf-6f9bd5507bbd -->

## What This Closes

Harness gap: the ternary precedence bug (commit a69909d) was latent because no test ever passed a cursor parameter to /api/feed. This DST rule ensures pagination correctness is continuously tested.

```bash
git diff --stat main..dst/feed-pagination-rule -- platform/backend/tests/
```

```output
 .../tests/simulation/rules/feed_pagination.py      | 146 +++++++++++++++++++++
 .../backend/tests/simulation/test_game_rules.py    |   4 +-
 2 files changed, 149 insertions(+), 1 deletion(-)
```

## Code Review Findings (CC harness)

CC's pre-commit review gate caught 2 bugs in the initial test code:

1. **Dead-code assert** in exhausted_cursor: guard made final assert unreachable, silently swallowing 500s. Fixed: assert unconditionally.
2. **String date comparison** in chronological: comparing ISO timestamps as strings is fragile. Fixed: parse with datetime.fromisoformat().

Also: added intra-page ordering validation, moved import to module top, renamed rules to drop test_ prefix (matches harness convention).

```bash
git show dst/feed-pagination-rule:platform/backend/tests/simulation/rules/feed_pagination.py
```
...
8678525 on branch dst/feed-pagination-rule, pushed to remote. 4 DST rules: no-overlap, chronological ordering (intra + cross page), exhausted cursor, cursor format.
